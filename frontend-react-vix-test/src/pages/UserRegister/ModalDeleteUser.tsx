...
export const ModalDeleteUser = ({
  ...
  userToDelete: IUserDB;
  onClose: () => void;
}) => {
  const { theme, mode } = useZTheme();
  const { t } = useTranslation();

  const { setUserDeleted } = useZColaboratorRegister();
  const { fetchListUsers } = useListUsers();
  const { deleteUser } = useUserResources();

  const onDelete = async () => {
    const response = await deleteUser(userToDelete.idUser);
    await fetchListUsers();
    setUserDeleted(response || null);
    onClose();
  };
  ...
  return (
    ...
    >
      ...
      >
        {`${t("colaboratorRegister.areYouSure")} ${userToDelete.username}?`}
      </TextRob20Font1M>
      <Box
        sx={{
          display: "flex",
